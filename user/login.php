...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $result=run_query("select `password` from `user` where `id`=?","s",$params);
    // $result=run_query_no_args("select `password` from `user`");

    $row=$result->fetch_assoc();
    if ($row==null)
        redirect_to("login.php?message=no account associated with this email");

    if(!password_verify($_POST['password'],$row['password']))
        redirect_to("login.php?email=".$_POST['email']."&message=password not matching");
...
<body>
    ...
    display: grid;
    align-items: center;"> Customer Login</h1>

    <h4>Don't have a account yet? <a href="signup.php">Sign Up</a></h4>
